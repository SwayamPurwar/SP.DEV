  ...
  return (
    <>
      ...
      <div ref={containerRef}>
        ...
        <header className="cs-hero" style={{ borderBottom: 'none', paddingBottom: '5vh' }}>
          <h1 className="cs-title" style={{ background: 'linear-gradient(180deg, #ffffff 0%, #888888 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            KITE ENGINE<br /><span className="outline-sub" style={{WebkitTextStroke: '1px #00E396', color: 'transparent'}}>ARCHITECTURE</span>
          </h1>
          <div className="cs-meta">
            <div className="meta-item"><h4>Latency</h4><p>{"< 50ms"}</p></div>
            <div className="meta-item"><h4>Role</h4><p>Systems Engineer</p></div>
            <div className="meta-item"><h4>Focus</h4><p>WebSockets & Redis</p></div>
            <div className="meta-item"><h4>Stack</h4><p>MERN + Socket.io</p></div>
          </div>
        </header>
        ...
        <div className="case-layout">
          ...
          <aside className="toc-sidebar">
            <a href="#architecture" onClick={(e) => scrollToSection(e, 'architecture')} className={`toc-link ${activeSection === 'architecture' ? 'active' : ''}`}>01. The Architecture</a>
            <a href="#execution" onClick={(e) => scrollToSection(e, 'execution')} className={`toc-link ${activeSection === 'execution' ? 'active' : ''}`}>02. Order Execution</a>
            <a href="#data" onClick={(e) => scrollToSection(e, 'data')} className={`toc-link ${activeSection === 'data' ? 'active' : ''}`}>03. Data Firehose</a>
          </aside>
          ...
          <div className="content-area">
            ...
            <section id="architecture" className="chapter">
              ...
              <div className="arch-diagram">
                <div className="arch-node" style={{ borderColor: '#00E396' }}>React Client<br/><span style={{fontSize: '0.7rem', color: '#888'}}>UI / Charting</span></div>
                <div className="arch-arrow">⟷</div>
                <div className="arch-node">Node.js API<br/><span style={{fontSize: '0.7rem', color: '#888'}}>Socket.io Server</span></div>
                <div className="arch-arrow">⟷</div>
                <div className="arch-node" style={{ borderColor: '#ff4560' }}>Redis<br/><span style={{fontSize: '0.7rem', color: '#888'}}>Pub/Sub Broker</span></div>
              </div>
            </section>

            {/* CHAPTER 2 */}
            <section id="execution" className="chapter">
              <h2><span>02 //</span> Order Execution</h2>
              <p style={{ color: '#b0b0b0', lineHeight: 1.8, marginBottom: '1.5rem' }}>
...
<span style={{color: '#c678dd'}}>const</span> processOrder = <span style={{color: '#c678dd'}}>async</span> (orderData) =&gt; {'{'}
  <span style={{color: '#5c6370'}}>// 1. Validate margin</span>
  <span style={{color: '#c678dd'}}>const</span> user = <span style={{color: '#c678dd'}}>await</span> User.findById(orderData.userId);
  <span style={{color: '#c678dd'}}>if</span> (user.margin &lt; orderData.requiredMargin) {'{'}
    <span style={{color: '#c678dd'}}>throw new</span> <span style={{color: '#e5c07b'}}>Error</span>(<span style={{color: '#98c379'}}>'Insufficient Funds'</span>);
  {'}'}

  <span style={{color: '#5c6370'}}>// 2. Publish to Redis Queue for Matching Engine</span>
  <span style={{color: '#c678dd'}}>await</span> redisClient.publish(<span style={{color: '#98c379'}}>'order_channel'</span>, <span style={{color: '#56b6c2'}}>JSON</span>.stringify(orderData));
  
  <span style={{color: '#5c6370'}}>// 3. Acknowledge receipt to client instantly</span>
  <span style={{color: '#c678dd'}}>return</span> {'{'} status: <span style={{color: '#98c379'}}>'PENDING'</span>, orderId: orderData.id {'}'};
{'}'};
                </div>
              </div>
                ...
                <div className="lt-body">
                  <div className="lt-order">
                    {orders.map((order, idx) => (
                      <div key={idx} className="eo-row" style={{ opacity: 1 - (idx * 0.1) }}>
                        <span style={{ color: '#555' }}>[{order.time}]</span>
                        <span style={{ color: '#a855f7' }}>{order.id}</span>
                        <span style={{ color: order.type === 'BUY' ? '#00E396' : '#ff4560' }}>{order.type}</span>
            ...
            <section id="data" className="chapter">
              <h2><span>03 //</span> The Data Firehose</h2>
              <p style={{ color: '#b0b0b0', lineHeight: 1.8, marginBottom: '1.5rem' }}>
