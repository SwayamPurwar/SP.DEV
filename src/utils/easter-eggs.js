...
let matrixInterval;
let gravityInterval; // Moved to top level for cleanup
...
export function initGravity() {
  ...
  const elements = document.querySelectorAll(
    "p, h1, h2, h3, span, img, .btn, .nav-item, .project-card",
  );
...
export function initEasterEggs() {
  ...
  if (!document.getElementById("blackout-overlay")) {
    ...
    // Let's attach this globally so we can remove it later
    window._blackoutMoveHandler = (e) => {
      if (blackoutDiv.classList.contains("active")) {
    ...
    if (e.key === "Escape") {
      ...
      if (escCount === 2) {
        bossScreen.style.display =
          bossScreen.style.display === "flex" ? "none" : "flex";
        document.title =
          bossScreen.style.display === "flex"
            ? "index.js - Visual Studio Code"
            : "Swayam | Creative Developer";
        escCount = 0;
      }
    }
  };
