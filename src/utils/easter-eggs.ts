...
// Extend Canvas element for our custom drops property
interface MatrixCanvas extends HTMLCanvasElement {
  drops?: number[];
}

export function toggleMatrix(enable: boolean) {
  if (typeof window === "undefined") return;
  
  const canvas = document.getElementById("matrix-canvas") as MatrixCanvas;
  if (!canvas) return;
...
function drawMatrix(ctx: CanvasRenderingContext2D, canvas: MatrixCanvas) {
  const characters = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  const fontSize = 14;
  if (!canvas.drops) {
    canvas.drops = new Array(Math.floor(canvas.width / fontSize)).fill(1);
  }
...
export function initGravity() {
  if (typeof document === "undefined" || gravityInterval) return;
  const elements = document.querySelectorAll<HTMLElement>("p, h1, h2, h3, span, img, .btn, .nav-item, .project-card");
  const gravityElements: GravityElement[] = [];
  ...
  elements.forEach((el) => {
    ...
    gravityElements.push({
      ...
      vy: 0,
      bounciness: Math.random() * 0.5 + 0.3,
    });
  });
  ...
  gravityInterval = setInterval(() => {
    gravityElements.forEach((obj) => {
      ...
      if (left < 0 || left + obj.element.offsetWidth > window.innerWidth) {
        obj.vx *= -1;
        left = left < 0 ? 0 : window.innerWidth - obj.element.offsetWidth;
      }
...
declare global {
  interface Window {
    _blackoutMoveHandler?: (e: MouseEvent) => void;
  }
}
...
export function initEasterEggs() {
  ...
  if (!document.getElementById("blackout-overlay")) {
    ...
    window._blackoutMoveHandler = (e: MouseEvent) => {
      if (blackoutDiv.classList.contains("active")) {
        blackoutDiv.style.setProperty("--x", e.clientX + "px");
        blackoutDiv.style.setProperty("--y", e.clientY + "px");
      }
    };
    document.addEventListener("mousemove", window._blackoutMoveHandler);
  }
  ...
  if (!bossScreen) {
   ...
   bossScreen.innerHTML = `<div class="fake-code"><span style="color: #ce9178">"use client"</span>;<br><br><span style="color: #c586c0">export</span> <span style="color: #c586c0">default</span> <span style="color: #569cd6">function</span> <span style="color: #dcdcaa">PortfolioPage</span>() {<br>&nbsp;&nbsp;<span style="color: #4ec9b0">console</span>.<span style="color: #dcdcaa">log</span>(<span style="color: #ce9178">"Project Deadline: ASAP"</span>);<br>&nbsp;&nbsp;<span style="color: #c586c0">return</span> (<br>&nbsp;&nbsp;&nbsp;&nbsp;&lt;<span style="color: #569cd6">main</span> className=<span style="color: #ce9178">"min-h-screen"</span>&gt;<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;<span style="color: #569cd6">h1</span>&gt;Compiling Next.js Build...&lt;/<span style="color: #569cd6">h1</span>&gt;<br>&nbsp;&nbsp;&nbsp;&nbsp;&lt;/<span style="color: #569cd6">main</span>&gt;<br>&nbsp;&nbsp;);<br>}<br><br><span style="color: #6a9955">// Press ESC twice to return to portfolio</span></div>`;
    document.body.appendChild(bossScreen);
  }

  let inputSequence = "", escCount = 0, escTimer: NodeJS.Timeout;
  ...
  const handleKeydown = (e: KeyboardEvent) => {
    ...
    if (e.key === "Escape") {
      ...
      if (escCount === 2 && bossScreen) {
        bossScreen.style.display = bossScreen.style.display === "flex" ? "none" : "flex";
        document.title = bossScreen.style.display === "flex" ? "index.tsx - Visual Studio Code" : "Swayam | Creative Developer";
        escCount = 0;
      }
    }
  };
  ...
  return () => {
    ...
    if (window._blackoutMoveHandler) {
      document.removeEventListener("mousemove", window._blackoutMoveHandler);
    }
