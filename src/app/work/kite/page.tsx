...
export default function Kite() {
  ...
  useEffect(() => {
    ...
    const bgCanvas = document.getElementById("market-chart") as HTMLCanvasElement;
    if (!canvas || !bgCanvas) return;

    const canvasCtx = canvas.getContext("2d");
    const bgCtx = bgCanvas.getContext("2d");
    if (!canvasCtx || !bgCtx) return; // Tells TS these aren't null

    let cw: number, ch: number, bw: number, bh: number, animationReq: number;
    let candles: { open: number; close: number; high: number; low: number; x: number }[] = [];
