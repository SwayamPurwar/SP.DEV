...
export default function Footer() {
  ...
  if (path === '/resume' || path === '/success' || path === '/404' || isWorkPage) {
      return null;
  }
  ...
  if (path === '/contact') {
    return (
      <footer id="footer" className="footer-contact" style={{ 
        minHeight: 'auto', 
        padding: '2rem 5vw', 
        background: 'transparent', // Blends with the contact page background
        borderTop: '1px solid rgba(255, 255, 255, 0.05)' 
      }}>
        <div className="footer-copyright" style={{ marginTop: '0', borderTop: 'none', paddingTop: '0' }}>
          <span>© {new Date().getFullYear()} Swayam Purwar. All rights reserved.</span>
          
          {/* Minimal Social Links for Contact Page */}
        

          <span style={{ fontFamily: 'var(--font-code)', color: '#666' }}>[ LOC: BHOPAL, IN ]</span>
        </div>
      </footer>
    );
  }
  ...
  if (path === '/about') {
    return (
      <footer id="footer" className="footer-about" style={{ minHeight: 'auto', paddingTop: '2rem' }}>
