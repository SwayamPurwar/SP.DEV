  ...
  if (cursor) {
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
      window.addEventListener("mousemove", (e) => {
        // --- NEW: Hide custom cursor when inside terminal ---
        const isTerminal = e.target.closest("#cmd-terminal");
        if (isTerminal) {
          cursor.style.opacity = 0; // Hide circle to show system cursor
          return; // Stop processing magnet effect
        } else {
